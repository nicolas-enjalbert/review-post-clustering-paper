...
data.frame(X2, Truth = as.factor(cl)) %>%
  ...
  geom_point(size = 1.9) +
  labs(x = "Dim 1", y = "Dim 2", color = "Clusters (HAC)") +
  ...
  guides(color = guide_legend(override.aes = list(size = 3)))  -> p_setting1_clustering
ggExtra::ggMarginal(p_setting1_clustering, type = "density", groupColour = TRUE,
                    groupFill = TRUE) -> p_setting1_clustering
...
for(i in seq(nb_experiments)){
  ...
  X2 <- SimuMultiCorr(n = n, MatMean = MatMean, Sigma = Sigma)
  cl <- cutree(hclust(dist(X2, method = "euclidean"), method = "ward.D2"), k = K)

  eta <- eta_comparison_mean(cl, 1, 2)
  pvalues[i] <- multivariate_Z_test(X2, eta, sigma)$pval
}

data.frame(pvalues = pvalues, code = "Naive-HAC") %>%
  ...
  labs(x = "p-values", color = "Methods", linetype = "Methods",
       y = "ECDF (through 500 experiments)") -> p_H0_ward
# p_H0_ward

ggarrange(p_setting1, p_setting1_clustering, p_H0_ward,
          # labels = c("A", "B"),
          ncol = 3, nrow = 1) -> p_naive
# p_naive
ggsave(plot = p_naive, "figures/Figure1.pdf",
       width = 12, height = 4)
