...
path <- sprintf(
  "results/simulation_multivariate_setting%s/Figure17_18/",
  setting
)
...
for (cc in seq_configs) {
  ...
  name_file <- sprintf(
    "sim_multi_set1_K=%s_a=%s_n=%s_p=%s_nb_experiment=%s.RDS",
...
K <- 2
pattern <- sprintf(
  "sim_multi_set1_K=%s_a=(.*)_n=(.*)_p=(.*)_nb_experiment=(.*).RDS",
  K
)
...
p_H0
ggsave(plot = p_H0, "figures/Figure17.pdf", height = 4, width = 8)


p_boxplot_alpha <- df_end %>%
  filter(a == 0) %>%
  mutate(alpha = as.factor(alpha)) %>%
  ...
  labs(y = "Accuracy rate", x = TeX("$\\gamma$"), color = TeX("$\\gamma$"))
p_boxplot_alpha
ggsave(
  plot = p_boxplot_alpha,
  filename = "figures/Figure18.pdf",
  height = 4,
  width = 8
)
