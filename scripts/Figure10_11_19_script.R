...
setting <- 2
path <- sprintf(
  "results/simulation_marginal_setting%s/Figure10_11_19/",
  setting
)
...
comparison_univariate_inference_methods(
  methods = c(
    "Z.test", "data.thinning", "Hivert", "Chen", "tn.test",
    "poclin"
  ),
  clusterings = c("HAC"),
  nb_experiments = 500,
...
p_power <- df_power %>%
  filter(n %in% c(100)) %>%
  filter(truesignal != "0") %>%
  filter(!(method_inference %in% c(
    "tn_test", "Naif - Z.test",
    "Hivert - direct_test - estimated"
  ))) %>%
  ...
  theme(legend.position = "bottom") +
  guides(color = guide_legend(nrow = 1)) +
  labs(y = "Statistical power", color = "Methods")
# p_power
ggsave(
  plot = p_power,
...
p_ARI <- df_power %>%
  ...
  theme(legend.position = "bottom") +
  guides(color = guide_legend(nrow = 1)) +
  labs(y = "Mean of ARI", color = "Methods")
# p_ARI
ggsave(
  plot = p_ARI,
