  ...
  for (cc in seq_configs) {
    ...
    ))
    {
      ...
      comparison %>%
        ...
        pivot_wider(
          names_from = c(X1), values_from = c(n),
          values_fill = list(n = 0)
        ) %>%
        mutate(n = -1) %>%
        pivot_wider(
          names_from = c(X2), values_from = c(n),
          values_fill = list(n = 0), names_repair = "minimal"
        ) -> df1
    ...
    res <- future.apply::future_lapply(1:nb_experiments, future.seed = TRUE, FUN = function(nexp) {
        ...
        for (cl_meth in clusterings) {
          for (test in TESTS_prime) {
            for (epsilon in epsilon_dts) {
              data_thinning <- pipeline_data_thinning(
                X = X, K = K, a = a,
      ...
      if ("Chen" %in% methods) {
        for (cl_meth in intersect(clustering, c("HAC", "kmeans"))) {{ if (cl_meth == "HAC") {
          clustering <- clustering_HAC
        } else if (cl_meth == "kmeans") {
          clustering <- clustering_kmeans
        }

        chen_marginal <- pipeline_chen_marginal(
          X = X, Sigma = Sigma,
          clustering = clustering$clustering,
          comparison = comparison,
          clustering_method = cl_meth,
    ...
    name_file <- sprintf(
      "sim_univariate_set%s_K=%s_a=%s_n=%s_p=%s_nb_experiment=%s_sigma=%s_rho=%s.RDS",
