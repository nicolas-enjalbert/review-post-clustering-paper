...
createSigma <- function(p, Sigma_design = c("independent", "toeplitz"),
                        max_sigma = 1,
                        diff_sigma = FALSE,
                        toeplitz_coeficient = 0.9){

  if(Sigma_design == "independent"){
    if(!diff_sigma){
      sigma <- rep(1, p)
    } else {
      sigma <- c()
      for(pp in names(table(truthF))){
        sigma <- c(sigma,seq(max_sigma, 0.1, length.out = table(truthF)[pp]))
      }
    }

    Sigma <- diag(sigma)

  } else if (Sigma_design == "toeplitz"){
    sigma = rep(max_sigma, p)
    Sigma <- toeplitz(toeplitz_coeficient^(0:(p-1))) * max_sigma
  }

  return(list(Sigma = Sigma, sigma = sigma))
}
...
signal_3clusters_eq <- function(n, a = 10, p = 3,
  ...
  K <- 3
  truthF <- rep(0, p)
  truthS <- sort(c(1:K, sample(1:K, size = n-K, replace = TRUE, prob = prob_clusters)))

  X <- matrix(0, nrow = n, ncol = p)
  signal <- matrix(c(-a/2,rep(0, p-1),
                     a/2,rep(0, p-1),
                     0,sqrt(3) * a /2, rep(0, p-2)), nrow = 3, byrow = TRUE)
  for (k in 1:K){
    indS <- which(truthS == k)
...
signal_2clusters <- function(n, p, a = 10, prob_clusters = c(0.5, 0.5),
  ...
  truthS <- sort(c(1:K,sample(1:K, size = n-2, replace = TRUE, prob = prob_clusters)))

  X <- matrix(0, nrow = n, ncol = p)
  signal <- matrix(c(rep(0, p),
                     rep(a, nb_variable_signal),
                     rep(0, p-nb_variable_signal)), nrow = 2, byrow = TRUE)
  for (k in 1:K){
    indS <- which(truthS == k)
    X[indS, ] <-  rep(signal[k,], each=length(indS))
  }
...
datathin_gaussian_multivariate <- function(X, epsilon, Sigma){
  ...
  for (i in 1:dim(X)[1]) {
    Xtr[i, ] <- MASS::mvrnorm(n = 1, mu = epsilon * X[i, ], Sigma = epsilon * (1 - epsilon) * Sigma)
  }
  Xte <- X - Xtr
...
truth_comparison_clusters <- function(clustering, MatMean, binary_comparison, schema_comparison = c("1vs1", "1vsothers")){
  n = length(clustering)
  if(schema_comparison == "1vs1"){
    data.frame(clustering = clustering, n = 1, indices = 1:n) %>%
      pivot_wider(names_from = clustering, values_from = c(n),
    ...
    truthF_comparison <- (abs(mean_p_K %*% binary_comparison[colnames(mean_p_K),] ))
  } else if (schema_comparison == "1vsothers") {
    truthF_comparison <- truthF
  }
  return(list(mean_p_K = mean_p_K, binary_n_K = binary_n_K, truthF_comparison = truthF_comparison))
}
...
clean_data <- function(df){
  df_clean <- df %>%
    mutate(method_clustering = replace(method_clustering, method_clustering == "ward", "HAC"))%>%
    ...
    mutate(code = paste(method_inference_abre, method_clustering_abre, sep = "-")) %>%
    ...
    mutate(method_clustering = replace(method_clustering, method_clustering == "HAC", "HAC (Ward's linkage)")) %>%
    mutate(method_clustering = factor(method_clustering,
                                      levels = c("HAC (Ward's linkage)", "K-means", "GMM")))

  df_clean %>% dplyr::select(method_inference_abre, method_clustering_abre, ndraws, code) %>%
    mutate(IS = ndraws != 0) %>% distinct() %>%
  ...
  linetype_palette <- setNames(df_color_linetype$linetype, df_color_linetype$code)
  ...
  df_clean %>%
    ...
    mutate(color = recode(method_inference_abre,
                          ...
                          "YFB" = "#00BFC4",
                          .default = "yellow")) %>%
    mutate(linetype = ifelse(IS, "dotted", "solid")) -> df_color_linetype_inf_IS
  color_palette_inf_IS <- setNames(df_color_linetype_inf_IS$color,
                                   df_color_linetype_inf_IS$code_inf_IS)
  linetype_palette_inf_IS <- setNames(df_color_linetype_inf_IS$linetype,
                                      df_color_linetype_inf_IS$code_inf_IS)
...
clean_data_estimation <- function(df){
  df %>%
    ...
    mutate(type_estimation_abre = recode(type_estimation,
                                         ...
                                         "unknown" = "")) %>%
    # mutate(method_clustering_abre = as.factor(method_clustering_abre)) %>%
    mutate(code = ifelse(method_inference_abre == "YFB", method_inference_abre, paste(method_inference_abre, type_estimation_abre, sep = "-"))) %>%
    mutate(code = ifelse(ndraws != 0, paste(code, "IS", sep = "-"), code)) %>%
    mutate(code_inf_IS = ifelse(ndraws != 0,
                                paste(method_inference_abre, "IS", sep = "-"),
                                method_inference_abre)) %>%
  ...
  df_clean %>% dplyr::select(method_inference_abre, method_clustering_abre,
    ...
    mutate(linetype = recode(type_estimation_abre,
                             ...
                             "all" = "dotted",
                             "IS" = "dotted",
                             .default = "solid"))-> df_color_linetype
...
clean_data_marginal <- function(df){
  df %>%
    ...
    group_by(nexp, K, n, p, ndraws, a, epsilon, method_inference, method_clustering,
    ...
    mutate(TV = factor(TV,
                       labels = c(
                         ...
                         "2" = expression(paste("signal: ", frac(a, 2), ",    ", j == 1)),
                         "3" = expression(paste("signal: ", sqrt(3) * frac(a, 2), ",    ", j == 2))
                       )
    )
    ) -> df_clean

  df_clean %>%
    dplyr::select(code) %>% distinct() %>% arrange(code) -> df_color_linetype
...
estimation_cov <- function(X, cl, rho){
  ...
  if (rho ==0){
    ...
    for(k in 1:K){
      ...
      sigma_k <- sum(rowMeans((scale(Xk, center = TRUE,
                                     scale = FALSE))^2))/(nk - 1)
