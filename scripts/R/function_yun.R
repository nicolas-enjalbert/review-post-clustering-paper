...
fun_proposed_exact = function(X, cl_meth) {
  # input(s):
  # - X: data matrix of dimensions n by q
  # output(s):
  # - p-value
  if(cl_meth == "ward"){
    ...
    cl = kmeans(X,centers = 2)$cluster
  }
  K = 2; k1 = 1; k2 = 2
  S = clusterpval:::compute_S_ward(X, hc, K, k1, k2, dist(X) ** 2)
  pval = fun_pval_sig_unknown(X, cl, S)
  return(list(pval = pval, cl = cl))
}
...
fun_proposed_approx = function(X, K, k1, k2, ndraws, alpha, cl_meth) {
  # input(s):
  # - X: data matrix of dimensions n by q
  ...
  # - acceptance rate
  if(cl_meth == "ward"){
    hc = hclust(dist(X) ** 2, method = "ward.D2")
    cl = cutree(hc, K)
  } else if (cl_meth == "kmeans"){
    cl = kmeans(X,centers = K)$cluster
  }
  ...
  n2 = sum(cl == k2)
  if(n1 <= 1 & n2 <= 1){
    return(list(pval = NA, acc_rate = NA, cl = cl))
  }
  m = n1 + n2
  P0 = fun_P0(cl, k1, k2)
  P1 = fun_P1(cl, k1, k2)
  ...
  for (j in 1:ndraws) {
    ...
    hc_y = hclust(dist(y) ** 2, method = "ward.D2")
    cl_y = cutree(hc_y, K)
  ...
  return(list(pval = pval, acc_rate = acc_rate, cl = cl))
}
