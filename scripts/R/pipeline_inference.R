pipeline_t.test <- function(X, clustering, comparison, MatMean ,
                            binary_comparison, schema_comparison = c("1vs1", "1vsothers")){

  n <- dim(X)[1]
  p <- dim(X)[2]
  nb_comparison <- dim(comparison)[1]

  clustering_comparison_truth <- truth_comparison_clusters(clustering = clustering,
                                                           MatMean = MatMean,
  ...
  for (k in 1:nb_comparison){
    if(all(comparison[k,] %in% clustering )){
      for (j in 1:p){
        ...
        vector_time_pvalue <- c(vector_time_pvalue, time_pvalue_stop - time_pvalue_start)
      }
    }
  }
  ...
  c(
    list(matrix_pvalues = matrix_pvalues,
...
pipeline_Z.test <- function(X, Sigma, clustering, comparison, MatMean, binary_comparison,  schema_comparison = c("1vs1", "1vsothers")){
  ...
  for (k in 1:nb_comparison){
    if(all(comparison[k,] %in% clustering )){
      eta <- eta_comparison_mean(clustering, k1 = comparison[k,1], k2 = comparison[k,-1])
      for (j in 1:p){
        time_pvalue_start <- Sys.time()
        Xj <- X[, j]
        sig <- Sigma[j,j]
        matrix_pvalues[j, k] <- univariate_Z_test(Xj, eta = eta, sig= sqrt(sig))$pval
        time_pvalue_stop <- Sys.time()
        vector_time_pvalue <- c(vector_time_pvalue, time_pvalue_stop - time_pvalue_start)
      }
    }
...
pipeline_hivert <- function(X, clustering, sigma,
                            ...
                            type_test = c("direct_test", "merge_test", "multimod_test"),
  ...
  for (k in 1:nb_comparison){
    for (j in 1:p){
      ...
      if(type_test == "direct_test"){
        matrix_pvalues[j, k] <- test_selective_inference(X = X, k1 = 1,
                                                         k2 = 2, g = j,
                                                         cl_fun = cl_fun,
                                                         cl = clust_arrange,
                                                         ndraws = ndraws,
        ...
        matrix_pvalues[j, k] <- merge_selective_inference(X = X, k1 = 1,
                                                          k2 = 2, g = j,
                                                          cl_fun = cl_fun,
                                                          cl = clust_arrange,
                                                          ndraws = ndraws
        ...
        matrix_pvalues[j, k] <- test_multimod(X = X, k1 = 1,
                                              k2 = 2, g = j,
                                              cl = clust_arrange
        )$pval
      }
...
pipeline_data_thinning <- function(X, K, a, Sigma, epsilon = 0.5,
  ...
  if(test == "t.test"){
    res_inference <- pipeline_t.test(X = Xinference, clustering = clustering,
                                     comparison = comparison,
                                     MatMean = MatMean,
                                     binary_comparison= binary_comparison,
                                     schema_comparison = schema_comparison)
  } else if (test == "Z.test"){
    res_inference <- pipeline_Z.test(X = Xinference, Sigma = (1-epsilon) * Sigma,
...
pipeline_chen_marginal <- function(X, K, a, Sigma,
  ...
  for (k in 1:nb_comparison){
    for (j in 1:p){
      ...
      if(clustering_method == "kmeans"){
        matrix_pvalues[j, k] <- kmeans_inference_1f(X = X, k = K_fix,
        ...
        matrix_pvalues[j, k] <- test_hier_clusters_exact_1f(X = X,
                                                            link = "ward.D",
                                                            hcl = hcl,
                                                            K = K_fix,
                                                            k1 = comparison[k, 1],
...
pipeline_poclin <- function(X, K, Sigma, aggregation = c("rescale", "walesiak"),
  ...
  for (k in 1:nb_comparison){
    ...
    eta[clustering %in% comparison[k, -1]] <- -1 / sum(clustering %in% comparison[k, -1])
    for(j in 1:p){
      ind <- 1:p == j
      kappa <- kronecker(eta, t(ind))
      matrix_pvalues[j,k] <- get_cond_p_value(X = X, kappa = kappa,
                                              clust = clust,
                                              lambda= lambda,
  ...
  c(
    list(matrix_pvalues = matrix_pvalues,
         ...
         time_inference = time_inference_stop - time_inference_start,
         comparison = comparison,
         parameters = c(test = "Poclin", method = "poclin",
...
pipeline_tn_test <- function(X, K, comparison,
  ...
  if (dim(comparison)[2] >2){
    ...
    for (k in comparison[,1]){
      ...
      b = SVC_k$intercept_
      pvals <- tn_test$tn_test(y=y, z=z, a=a, b=b,
    ...
    for (k in 1:nb_comparison){
      y = matrix(Xinference[which(labelsInference == comparison[k,1]),], ncol = p)
      z = matrix(Xinference[which(labelsInference == comparison[k,2]),], ncol = p)
  ...
  clustering_comparison_truth <- truth_comparison_clusters(clustering = clustering_final,
                                                           MatMean = MatMean,
                                                           binary_comparison = binary_comparison,
                                                           schema_comparison = schema_comparison
  )
  ...
  c(
    list(matrix_pvalues = matrix_pvalues,
         time_mean_pvalue = (time_inference_stop - time_inference_start)/(nb_comparison*p),
         time_inference = time_inference_stop - time_inference_start,
         comparison = comparison,
         parameters = c(test = "Tn_test", method = "Tn_test",
