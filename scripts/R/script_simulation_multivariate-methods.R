...
comparison_multivariate_inference_methods <- function(
    methods = c("naif", "data.thinning", "YFB", "Cond", "Cond-GD"),
  ...
  for (cc in seq_configs) {
    ...
    print(sprintf(
      "a = %s ; n = %s ; p = %s ; sigma = %s ; rho = %s",
      a, n, p, sigma, rho
    ))


    res <- future.apply::future_lapply(1:nb_experiments,
      future.seed = TRUE,
      FUN = function(nexp) {
        {
          if (setting == 1) {
        ...
        ###### Conditional approach of Gao et al (2024)
        if ("Cond" %in% methods) {
          if (rho == 0) {
            if ("HAC" %in% clusterings) {
              for (method in HACMethods) {
                ...
                for (type_cov in type_estimation) {
                  ...
                  if (0 %in% Ndraws) {
                    t1 <- Sys.time()
                    tree <- hclust(dist(X, method = "euclidean")^2,
                      method = method
                    )
                    Gao_exact <- test_hier_clusters_exact(
                      X = X, link = method,
                      hcl = tree, K = K, k1 = 1,
                    ...
                    df <- rbind(df, data.frame(
                      nexp = nexp, K = K, n = n, p = p, ndraws = 0,
                      a = a, epsilon = 0,
                      method_inference = "Gao",
            ...
            if ("kmeans" %in% clusterings) {
              ...
              for (type_cov in type_estimation) {
                ...
                for (ndraws in Ndraws[which(Ndraws != 0)]) {
                  t1 <- Sys.time()
                  kmeans_MC <- test_clusters_approx(X,
                    k1 = 1, k2 = 2, iso = iso, sig = sigma_used,
                    ndraws = ndraws, cl_fun = cl_kmeans
                  )
                  time_ <- difftime(Sys.time(), t1, units = "secs")
                  ARI <- mclust::adjustedRandIndex(kmeans_MC$clusters, truthS)
                  df <- rbind(df, data.frame(
                    nexp = nexp, K = K, n = n, p = p, ndraws = ndraws,
                    a = a, epsilon = 0,
                    method_inference = "Gao",
                ...
                if (0 %in% Ndraws) {
                  t1 <- Sys.time()
                  kmeans_exact <- kmeans_inference(
                    X = X, k = K, cluster_1 = 1, cluster_2 = 2,
                    iso = iso, sig = sigma_used, iter.max = 1000,
                    seed = seed, verbose = TRUE
                  )
                  time_ <- difftime(Sys.time(), t1, units = "secs")
                  ARI <- mclust::adjustedRandIndex(
                    kmeans_exact$final_cluster,
                    truthS
                  )
                  df <- rbind(df, data.frame(
                    nexp = nexp, K = K, n = n, p = p, ndraws = 0,
            ...
            if ("GMM" %in% clusterings) {
              GMMclustering <- function(X) {
                ...
                res0 <- mixmodCluster(as.data.frame(X),
                  nbCluster = K,
                  models = model
                )
            ...
            if ("HAC" %in% clusterings) {
              ...
              for (type_cov in type_estimation) {
                ...
                t1 <- Sys.time()
                cond_GD_exact <- test.clusters.hc(X, U, Sigma_used,
                  NC = 2, clusters = c(1, 2),
                  plot = FALSE, linkage = "ward.D"
                )
                time_ <- difftime(Sys.time(), t1, units = "secs")
                ARI <- mclust::adjustedRandIndex(cond_GD_exact$hcl, truthS)
                df <- rbind(df, data.frame(
                  nexp = nexp, K = K, n = n, p = p, ndraws = 0,
                  a = a, epsilon = 0,
                  ...
                  sigma = sigma, rho = rho,
                  type_estimation = type_cov,
                  pvaleur = cond_GD_exact$pvalue,
                  stat = cond_GD_exact$stat,
                  ARI = ARI, time = as.numeric(time_)
                ))
              }
            }
        ...
        if ("YFB" %in% methods) {
          ...
          for (ndraws in Ndraws[Ndraws != 0]) {
            t1 <- Sys.time()
            yun_ISMC <- fun_proposed_approx(X,
              K = K, k1 = 1, k2 = 2, ndraws = ndraws,
        ...
        if ("data.thinning" %in% methods) {
          if ("known" %in% type_estimation) {
            for (epsilon_dt in epsilon_dts) {
              t1_dt <- Sys.time()
              XDT <- datathin_gaussian_multivariate(X,
                epsilon = epsilon_dt,
                Sigma = Sigma
              )
              Xclustering <- XDT$Xtr
              ...
              if ("kmeans" %in% clusterings) {
                ...
                eta <- eta_comparison_mean(
                  clustering = clustering, k1 = 1,
                  k2 = 2
                )
                naif_kmeans <- multivariate_Z_test(
                  eta = eta, X = Xtest,
                  sig = sqrt(1 - epsilon_dt) *
                    sigma
                )
                time_ <- difftime(Sys.time(), t1, units = "secs")
                ARI <- mclust::adjustedRandIndex(clustering, truthS)
              ...
              if ("HAC" %in% clusterings) {
                t1 <- Sys.time()
                clustering <- cutree(hclust(dist(Xclustering),
                  method = "ward.D2"
                ), k = K)
                eta <- eta_comparison_mean(
                  clustering = clustering, k1 = 1,
              ...
              if ("GMM" %in% clusterings) {
                t1 <- Sys.time()
                model <- mixmodGaussianModel(listModels = c("Gaussian_p_L_I"))
                res0 <- mixmodCluster(as.data.frame(Xclustering),
                  nbCluster = K,
                  models = model
                )
                clustering <- res0@bestResult@partition
    ...
    name_file <- sprintf(
      "sim_multi_set%s_K=%s_a=%s_n=%s_p=%s_nb_experiment=%s_sigma=%s_rho=%s.RDS",
