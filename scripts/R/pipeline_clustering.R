pipeline_kmeans <- function(X, K){
  ...
  return(list(clustering = clustering, time_clustering = time_clustering_stop - time_clustering_start))
}

pipeline_HAC <- function(X, K, distance = "euclidean", linkage = "ward.D2"){
  time_clustering_start <- Sys.time()
  clustering <- cutree(hclust(dist(X, method = distance), method = linkage), k = K)
  time_clustering_stop <- Sys.time()
